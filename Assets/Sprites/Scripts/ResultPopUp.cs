...
public class ResultPopUp : MonoBehaviour
{
    ...
    private void DispayResult(int money, int battleTime, int remainingballCount, bool isClear)
    {
        ...
        sequence.Append(DOTween.To(() => initValue,
                    ...
                    isClear ? battleTime * magnifyingTimePoint : 0,　　　　　//　<=　三項演算子を利用して、ステージクリアかゲームオーバーか処理を分岐させて、ボーナス値を制御している
        ...
        sequence.Append(DOTween.To(() => initValue,
                    ...
                    isClear ? remainingballCount * magnifyingBallPoint : 0,　　　//　<=　三項演算子を利用して、ステージクリアかゲームオーバーか処理を分岐させて、ボーナス値を制御している
                    1.0f).OnComplete(() => { initValue = 0; }).SetEase(Ease.InCirc));
